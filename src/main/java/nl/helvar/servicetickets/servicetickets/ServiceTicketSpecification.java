...
public class ServiceTicketSpecification {
    ...
    public static Specification<ServiceTicket> ticketPriorityEquals(String ticketPriority) {
        try {
            TicketPriority priority = TicketPriority.valueOf(ticketPriority.toUpperCase());
            return (root, query, builder) -> builder.equal(root.get("priority"), priority);
        } catch (IllegalArgumentException ex) {
            throw new InvalidEnumConstantException(TicketPriority.values());
        }
    }
